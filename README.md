...
**版本**: v1.7.4 | **Node.js**: ≥18.0.0 | **许可证**: MIT | **Docker Hub**: `bbblq/interactive-map`
...
- **四种标记类型**:
  ...
  - **形状标记**: 支持矩形、圆形和箭头，可调整填充、描边、透明度、旋转与层级。
  - **填色标记 (v1.6.7)**: 像画图软件一样依次点击顶点并闭合自定义多边形，统一归入独立的“区域标记”分类，支持拖动 / 删除顶点、重新绘制、填充与描边透明度，以及九宫格文字位置。
...
- **标记锁定 (v1.6.3)**: 后台可从标记列表或右键菜单锁定图层；锁定后在地图上保持原始外观并完全忽略鼠标事件，只能从侧边栏解锁，同时支持撤销 / 重做和 SQLite 持久化。
...
3. **添加标记**: 进入"标记管理"标签页，在地图上右键选择图标、形状或"绘制填色标记"
...
- **绘制填色区域**: 右键选择"绘制填色标记"，依次点击顶点；点击第一个顶点或双击完成闭合，随后自动打开样式表单
- **编辑填色区域**: 在填色标记表单中点击"绘制 / 编辑顶点"，拖动圆点调整形状，右键圆点删除，或点击"重新绘制"从头绘制
...
### v1.6.7 (2026-07)
- ✨ 新增"填色标记"类型：自由绘制并闭合多边形区域，支持填色、描边、透明度、旋转、层级和锁定
- ✨ 支持九宫格文字位置、重新绘制和已有顶点拖动 / 删除，闭合后自动打开样式表单
- ✨ 前台、后台、图片导出和 PDF 导出统一渲染填色区域

### v1.6.4 (2026-07)
- 🐛 锁定标记在地图上不再显示轮廓或锁角标，并完全退出鼠标命中，与背景图片自然融合

### v1.6.3 (2026-07)
- ✨ 新增后台标记锁定：锁定后阻止选中、拖动、缩放、旋转、编辑和删除
- ✨ 右键菜单支持锁定，侧边栏支持锁定 / 解锁；地图上的锁定标记保持原始外观并完全忽略鼠标事件
- ↶ 锁定状态接入撤销 / 重做，并持久化到 SQLite
